...
def email_subject = 'Hygge EV UAT Server Deployment'
def skipStage = false
    ...
    stages
    {
        ...
        stage('Build')
        {
            steps
            {
                ...
                rm -rf artifacts
                mkdir -p artifacts
                zip -r artifacts/hygge-ev-backend-python-uat.zip . -x '.git*' -x 'artifacts*'
                scp -o StrictHostKeyChecking=no artifacts/hygge-ev-backend-python-uat.zip hygge-ev-user@15.207.53.80:.
                """
            }
        }
        ...
        stage('Deploy')
        {
            steps
            {
                ...
                process=$(ps -ef | grep '8004' | grep -v 'grep' | awk '{print$2}' | wc -l)
                if [ "$process" -gt 1 ]
                then
                {
                kill -9 $(ps -ef | grep '8004' | grep -v 'grep' | awk '{print$2}')
                }
                fi
                mv ~/application-uat/server.log ~/logs-backup/server_uat_$(date +"%Y%m%d_%H%M%S").log
                rm -rf ~/application-uat/*
                unzip -o ~/hygge-ev-backend-python-uat.zip -d ~/application-uat/hygge-ev-backend-python/
                #pip install -r ~/application-uat/hygge-ev-backend-python/requirements.txt
                export JENKINS_NODE_COOKIE=dontKillMe
                cd ~/application-uat/hygge-ev-backend-python/
                /usr/bin/nohup /home/hygge-ev-user/.pyenv/shims/gunicorn app.main:application -b 0.0.0.0:8004 --env APP_ENV=uat > ~/application-uat/server.log &
                exit 0
                EOF
                '''
            }
