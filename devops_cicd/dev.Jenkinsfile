...
def email_subject = 'Hygge EV DEV Server Deployment'
def skipStage = false
    ...
    stages
    {
        ...
        stage('Build')
        {
            steps
            {
                ...
                rm -rf artifacts
                mkdir -p artifacts
                zip -r artifacts/hygge-ev-backend-python-dev.zip . -x '.git*' -x 'artifacts*'
                scp -o StrictHostKeyChecking=no artifacts/hygge-ev-backend-python-dev.zip hygge-ev-user@15.207.53.80:.
                """
            }
        }
        ...
        stage('Deploy')
        {
            steps
            {
                ...
                process=$(ps -ef | grep '8002' | grep -v 'grep' | awk '{print$2}' | wc -l)
                if [ "$process" -gt 1 ]
                then
                {
                kill -9 $(ps -ef | grep '8002' | grep -v 'grep' | awk '{print$2}')
                }
                fi
                mv ~/application-dev/server.log ~/logs-backup/server_dev_$(date +"%Y%m%d_%H%M%S").log
                rm -rf ~/application-dev/*
                unzip -o ~/hygge-ev-backend-python-dev.zip -d ~/application-dev/hygge-ev-backend-python/
                #pip install -r ~/application-dev/hygge-ev-backend-python/requirements.txt
                export JENKINS_NODE_COOKIE=dontKillMe
                cd ~/application-dev/hygge-ev-backend-python/
                /usr/bin/nohup /home/hygge-ev-user/.pyenv/shims/gunicorn app.main:application -b 0.0.0.0:8002 --env APP_ENV=dev > ~/application-dev/server.log &
                exit 0
                EOF
                '''
            }
